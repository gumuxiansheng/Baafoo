...
Pulsar **2.7.4** 客户端专项测试应用。结构与 `baafoo-test-spring` 对齐，但将 `pulsar-client` 硬锁定到 **2.7.4**——即腾讯 TDMQ for Pulsar 所基于的 Pulsar 版本，用来验证 Agent 与 `baafoo-plugin-tdmq` 在 2.7.4 线协议下的拦截/重定向是否正确。
...
`baafoo-plugin-tdmq` 自称 "TDMQ / Pulsar 2.7.4 protocol-adapter" 并把非本地 broker 重定向到 `localhost:9005`，但仓库内此前从未用真正的 2.7.4 client 跑过端到端验证。本模块补齐这一空缺。
...
产物：`baafoo-test-pulsar/target/baafoo-test-pulsar-1.0.0-SNAPSHOT.jar`
...
java -javaagent:baafoo-agent/target/baafoo-agent-1.0.0-SNAPSHOT.jar=config=baafoo-agent/src/main/resources/baafoo-agent.yml \
     -jar baafoo-test-pulsar/target/baafoo-test-pulsar-1.0.0-SNAPSHOT.jar
```

> Java 9+ 需追加：`--add-opens java.base/java.net=ALL-UNNAMED`

Agent 会拦截 `ClientBuilder.serviceUrl(...)`，把 `pulsar://pulsar-broker:6650` 重写到 `pulsar://<host>:9003`（内置 PulsarMockBroker）。若 `plugins/baafoo-plugin-tdmq.jar` 在位，则非本地 broker 会被进一步重定向到 `localhost:9005`。

### 方式二：不挂 Agent

```bash
java -jar baafoo-test-pulsar/target/baafoo-test-pulsar-1.0.0-SNAPSHOT.jar
```
...
>     "tdmqPlugin": "pulsar://localhost:9005 (baafoo-plugin-tdmq)"
>   }
> }
> ```
...
4. 应用日志应出现：
   ...
   ```
5. 将 `plugins/baafoo-plugin-tdmq.jar` 放到位再重启应用，触发 `/send`，日志应出现：
   ```
