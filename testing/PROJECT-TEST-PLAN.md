...
12. [前端测试计划](#12-前端测试计划)
    - 12.5 [前端单元测试](#125-前端单元测试)
    - 12.6 [前端响应式与可访问性测试](#126-前端响应式与可访问性测试)
    - 12.7 [前端视觉回归测试](#127-前端视觉回归测试)
13. [安全测试计划](#13-安全测试计划)
...
| Service-Name DNS | ✅ 支持 | InetAddress | ServiceNameDnsAdvice |
...
> **注**: §5.4（单元测试执行计划）和 §5.5（覆盖率追踪）暂缺，待后续补充。§5.6 变异测试已实现在线配置，参见父 POM `pluginManagement` 中的 `pitest-maven` 插件。
...
| UT-AGENT-021 | ServiceNameDnsAdvice - 服务名 DNS 拦截 | P1 | 待补充 |
...
.\testing\run-mutation.ps1

# 运行单个模块
.\testing\run-mutation.ps1 -Module baafoo-server
```
...
项目已配置 `maven-toolchains-plugin`（在 `pom.xml` 中），将 `testing\toolchains.xml` 复制到 `~/.m2\toolchains.xml` 并编辑 JDK 路径即可：

```powershell
Copy-Item testing\toolchains.xml $env:USERPROFILE\.m2\toolchains.xml
```
...
企业级应用测试配置位于 `testing/enterprise/` 目录，按行业领域组织：
...
| **预期结果** | 1. 响应 200，body 为 Mock 数据<br>2. Agent 日志含 `HttpOpenServerAdvice redirect` 和 `ServiceNameDns redirect`<br>3. Server 端无 `No Baafoo rule matched` 日志<br>4. JaCoCo/SkyWalking 不干扰 Baafoo 的 Socket/HttpClient 拦截 |
| **通过标准** | 全部预期结果满足 |
...
| java.net.InetAddress 增强冲突 | SkyWalking 增强 HTTP 客户端相关类，Baafoo 通过 ServiceNameDns*Advice 增强 `java.net.InetAddress`，可能冲突 | 通过 MULTI-008 专项检测 |
...
# 企业级应用测试（统一管理脚本）
cd testing\enterprise && .\enterprise-env.ps1 -Action start -Apps kafka,petclinic
.\run-all-smoke-tests.ps1 -Apps kafka,petclinic
