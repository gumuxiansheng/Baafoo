...
应用进程 JVM
  ├─ Agent Core (App CL)
  │    ├─ Byte Buddy Advice (内联到 Bootstrap CL)
  │    ├─ PluginManager (App CL)
  │    └─ GlobalRouteState (Bootstrap CL)
  │
  ├─ Plugin JAR (PluginClassLoader, parent=null)
  │    └─ YourPlugin implements AgentPlugin
  │
  └─ 宿主应用依赖 (App CL, 与插件完全隔离)
```

**核心设计原则**：
- Agent Core 只负责"拦不拦"（路由查找 + 地址重写）
- 插件负责"怎么模拟"（协议特定的处理逻辑）
- 插件的 ClassLoader `parent=null`，与宿主应用的同名依赖（如 pulsar-client）完全不冲突
