...
**参考测试计划**: `testing/PROJECT-TEST-PLAN.md` 第 1051-1100 行 Spring Cloud Alibaba 测试场景
...
| [Provider application.yml](file:///c:/Dev/Projects/Baafoo/testing/enterprise/spring-cloud-alibaba/baafoo-sca-test/baafoo-sca-provider/src/main/resources/application.yml) | - | 关键修复：Nacos 注册 ip=provider |
...
**修复文件**: [Provider application.yml](file:///c:/Dev/Projects/Baafoo/testing/enterprise/spring-cloud-alibaba/baafoo-sca-test/baafoo-sca-provider/src/main/resources/application.yml)
...
**修复脚本**: `testing/tmp/restore-mock-rule-host.ps1`
...
**解决方案**: 创建 `testing/tmp/create-sca-envs.ps1` 脚本，去除 tags 字段重新创建环境。
...
cd testing\enterprise\spring-cloud-alibaba
docker compose -f ../common/docker-compose.base.yml -f docker-compose.yml up -d --build
...
# 执行企业级测试脚本
powershell -ExecutionPolicy Bypass -File testing\tmp\run-sca-enterprise-test.ps1

# 创建 Baafoo 环境（仅首次运行）
powershell -ExecutionPolicy Bypass -File testing\tmp\create-sca-envs.ps1

# 重新创建带 host 字段的 Mock 规则（仅首次运行）
powershell -ExecutionPolicy Bypass -File testing\tmp\restore-mock-rule-host.ps1
...
测试结果原始数据已保存至 `testing/tmp/sca-test-results.csv`：
