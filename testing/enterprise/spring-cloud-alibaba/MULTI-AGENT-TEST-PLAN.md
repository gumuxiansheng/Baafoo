...
| **预期结果** | 1. 响应 200，body=`hello Nacos Discovery mock via serviceName`<br>2. Agent 日志含 `ConsulHttpAdvice redirect` 和 `ConsulDns redirect`<br>3. Server 端无 `No Baafoo rule matched` 日志<br>4. JaCoCo/SkyWalking 不干扰 Baafoo 的 Socket/HttpClient 拦截 |
| **通过标准** | 全部预期结果满足 |
...
2. **java.net.InetAddress 增强冲突**: SkyWalking 增强 HTTP 客户端相关类，Baafoo 通过 ConsulDns*Advice 增强 `java.net.InetAddress`，可能冲突
