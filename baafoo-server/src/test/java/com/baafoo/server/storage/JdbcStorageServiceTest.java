...
public class JdbcStorageServiceTest {
    ...
    public void userCrudViaFacade() {
        ...
        assertTrue(storage.updateUserRole("admin", "tester"));
        assertEquals("tester", storage.getUserByUsername("admin").getRole());
        ...
        assertEquals("new-hash", storage.getUserByUsername("admin").getPassword());
    ...
    @Test
    public void userPhoneAvatarExternalIdPersistedViaFacade() {
        ...
        user.setAvatar("https://example.com/a/1.png");
        User created = storage.createUser(user);
        assertNotNull(created);

        User fetched = storage.getUserByUsername("alice");
        assertNotNull(fetched);
        assertEquals("13800138000", fetched.getPhone());
        assertEquals("https://example.com/a/1.png", fetched.getAvatar());

        // external_id is generated by the DB; UuidTypeHandler must materialize
        // it as a String so User.externalId (String) does not blow up on
        // PostgreSQL drivers that return java.util.UUID from getObject().
        assertNotNull("externalId must be populated by UuidTypeHandler", fetched.getExternalId());
        assertEquals(36, fetched.getExternalId().length()); // canonical UUID form

        // Sanity: empty phone/avatar path still works (NULL allowed).
        User minimal = new User();
        minimal.setUsername("bob");
        minimal.setPassword("hashed");
        minimal.setRole("guest");
        storage.createUser(minimal);
        User bob = storage.getUserByUsername("bob");
        assertNotNull(bob);
        assertNull(bob.getPhone());
        assertNull(bob.getAvatar());
