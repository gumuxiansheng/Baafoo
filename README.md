...
├── stop-server.sh        # Server 停止脚本 (Linux/macOS)
├── stop-server.bat       # Server 停止脚本 (Windows)
...
| 目录/文件 | 说明 |
|:----------|:-----|
| `testing/2_IntegrationTest/rules/` | 60+ 个 JSON 规则文件，覆盖全部协议与条件类型 |
...
| `rulesFilePath` | — | 规则文件路径（本地规则文件加载，Agent 启动时加载） |
...
```javascript
const { Client } = require('@baafoo/sdk');
...
// HTTP 拦截（自动重定向到 Baafoo Server）
const { patch } = require('@baafoo/sdk/intercept');
patch(client);
```
...
client := baafoo.New(baafoo.Options{
    ServerURL:   "http://localhost:8084",
    Environment: "default",
    APIKey:      "dev-key-001",
})
