...
function Export-JUnitXml {
    ...
    $null = $sb.AppendFormat('<testsuites name="baafoo-enterprise-kafka" tests="{0}" failures="{1}" skipped="{2}" errors="0">', $script:TestResults.Count, $script:failed, $script:skipped)
    $null = $sb.AppendFormat('<testsuite name="EnterpriseKafka" tests="{0}" failures="{1}" skipped="{2}" errors="0" timestamp="{3}">', $script:TestResults.Count, $script:failed, $script:skipped, $ts)
    foreach ($t in $script:TestResults) {
        ...
        $null = $sb.AppendFormat('<testcase name="{0}" classname="EnterpriseKafka" status="{1}">', $nameEsc, $t.Status)
        if ($t.Status -eq "fail") {
