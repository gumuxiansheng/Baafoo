...
| 属性 | 内容 |
|---|---|
| **描述** | 模拟 Kafka Broker 的协议子集，在独立端口（默认 9002）监听。v1 明确覆盖以下 API：**Metadata**（集群元数据查询）、**Produce**（生产者发送消息）、**Fetch**（消费者拉取消息）。这三个 API 覆盖了 Kafka Producer/Consumer 的核心运行路径，确保应用在 Mock 环境下的 Producer `send()` 和 Consumer `poll()` 均可正常工作。 |
...
| **AC-07** | Mock Broker 启动时日志明确列出已支持的 API 列表和版本范围 |
