...
# NOTE: L2-RULESET-003~005 (Update/Disable/Re-enable) removed — server does not
# implement PUT /api/rulesets/{id}. RuleApiHandler only supports POST/GET/DELETE.
# When the server adds PUT support, restore update/disable/re-enable tests here.

# L2-RULESET-003: Delete rule set and verify removal
