...
serverUrl: "http://127.0.0.1:8080"   # Baafoo Server 地址
...
```yaml
httpPort: 8080                       # 管理 API + Web 控制台端口
protocolPorts:
...
```bash
# 创建环境
curl -X POST http://localhost:8080/__baafoo__/api/environments \
  -H 'Content-Type: application/json' \
  -d '{"name":"ft-1","mode":"stub","description":"FT-1 挡板自测环境"}'

# 切换模式
curl -X PUT http://localhost:8080/__baafoo__/api/environments/ft-1 \
  -H 'Content-Type: application/json' \
  -d '{"mode":"passthrough"}'
```
...
# 撤销规则修改
curl -X POST http://localhost:8080/__baafoo__/api/rules/{id}/undo
```
...
```bash
# 创建场景集
curl -X POST http://localhost:8080/__baafoo__/api/scenes \
  -H 'Content-Type: application/json' \
...
# 启动开发服务器（端口 3000，自动代理 API 到 localhost:8080）
npm run dev
