  ...
  <div class="rule-editor-page">
    ...
    <el-form :model="form" label-width="100px" size="small" v-if="isNew || rule" v-loading="loading">
      ...
      <el-card shadow="never" class="section-card">
        <template #header>
          <div class="section-header">
            <span class="section-title">{{ $t('rules.basicInfo') }}</span>
            <el-form-item :label="$t('rules.enabled')" class="inline-switch">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item :label="$t('rules.fakerSeed')">
              ...
              <div class="form-hint">{{ $t('rules.fakerSeedHint') }}</div>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item :label="$t('rules.counterReset')">
              ...
              <div class="form-hint">{{ $t('rules.counterResetHint') }}</div>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-if="!isNew && form.protocol === 'http'">
            <el-form-item :label="$t('rules.stateReset')">
              ...
              <div class="form-hint">{{ $t('rules.stateResetHint') }}</div>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item :label="$t('rules.effectiveEnvLabel')">
          <div v-if="inheritedEnvs.length > 0" class="inherited-envs">
            <el-tag v-for="env in inheritedEnvs" :key="env" size="small" type="warning" disable-transitions>
          ...
          <div class="form-hint">
            {{ $t('rules.envNoEffect') }}
            <span v-if="inheritedEnvs.length > 0" class="inherited-hint">；{{ $t('rules.envInherited') }}</span>
          </div>
        </el-form-item>
      </el-card>
      ...
      <el-card shadow="never" class="section-card">
        <template #header>
          <div class="section-header">
            <span class="section-title">{{ $t('rules.matchConditions') }}</span>
            <div class="section-actions">
              ...
              <el-button size="small" type="primary" plain @click="addCondition" v-if="authStore.canWriteRule">{{ $t('rules.addCondition') }}</el-button>
            </div>
          </div>
        </template>
        ...
        <div v-if="form.conditions.length === 0" class="empty-hint">
          {{ $t('rules.noConditionMatch') }}
        </div>

        <div v-for="(cond, idx) in form.conditions" :key="'cond-' + idx" class="condition-row">
          <el-row :gutter="10" align="middle">
            ...
            <el-col :span="cond.type === 'header' || cond.type === 'query' ? 9 : 13" v-if="cond.operator !== 'exists'">
              <el-input v-model="cond.value" size="small" :placeholder="$t(getValuePlaceholder(cond))" />
            </el-col>
            <el-col :span="4" class="condition-actions">
      ...
      <el-card shadow="never" class="section-card">
        <template #header>
          <div class="section-header">
            <span class="section-title">{{ $t('rules.responseBranches') }}</span>
            <el-button size="small" type="primary" plain @click="addResponse" v-if="authStore.canWriteRule">{{ $t('rules.addResponse') }}</el-button>
          </div>
        </template>

        <div v-if="form.responses.length === 0" class="empty-hint">
          {{ $t('rules.addAtLeastOne') }}
        </div>

        <div v-for="(resp, idx) in form.responses" :key="'resp-' + idx" class="response-card">
          <div class="response-card-header">
            <div class="response-title-wrap">
              <span class="response-card-title">{{ $t('rules.responseName') }} #{{ idx + 1 }}</span>
              <el-input v-model="resp.name" size="small" :placeholder="$t('rules.responseNamePlaceholder')" class="response-name-input" />
            </div>
            <div class="response-card-tags">
              ...
              <el-button size="small" type="danger" text @click="removeResponse(idx)" v-if="authStore.canWriteRule">{{ $t('rules.deleteResponse') }}</el-button>
            </div>
          </div>
          ...
          <!-- Response Condition -->
          <div class="response-subsection">
            <div class="subsection-header">
              <span class="subsection-title">{{ $t('rules.responseMatchCondition') }}</span>
              <div>
                ...
                <span v-if="!resp.condition" class="default-hint">{{ $t('rules.defaultResponseHint') }}</span>
              </div>
            </div>
            <div v-if="resp.condition" class="subsection-body">
              <el-row :gutter="10" align="middle">
                <el-col :span="4">
                ...
                <el-col :span="resp.condition.type === 'header' || resp.condition.type === 'query' ? 9 : 13" v-if="resp.condition.operator !== 'exists'">
          ...
          <div class="response-subsection">
            <div class="subsection-header">
              <span class="subsection-title">{{ $t('rules.responseHeaders') }}</span>
              <el-button size="small" @click="addResponseHeader(resp)">{{ $t('rules.addHeader') }}</el-button>
            </div>
            <div class="subsection-body">
              <div v-for="(h, hIdx) in getResponseHeaders(resp)" :key="'h-' + hIdx" class="header-row">
                <el-row :gutter="10" align="middle">
                  <el-col :span="7">
          ...
          <div class="response-subsection">
            <div class="subsection-header">
              <span class="subsection-title">{{ $t('rules.responseBody') }}</span>
            </div>
            <div class="subsection-body">
              ...
              <div class="faker-ref-toggle">
                <el-button size="small" text @click="showFakerRef = !showFakerRef">
                  {{ showFakerRef ? $t('rules.templates.collapse') || '收起 Faker 模板' : ($t('rules.templates.expand') || '展开 Faker 模板') }}
                </el-button>
              </div>
      ...
      <el-card shadow="never" class="section-card">
        <template #header>
          <div class="section-header">
            <span class="section-title">{{ $t('rules.faultInjection') }}</span>
            <el-switch v-model="form.faultInjectionEnabled" size="small" v-if="authStore.canWriteRule" />
          </div>
        </template>

        <div v-if="form.faultInjectionEnabled" class="fault-injection-body">
          <div class="form-hint">{{ $t('rules.faultHint') }}</div>
          <div v-for="(fault, fIdx) in form.faults" :key="'fault-' + fIdx" class="fault-card">
            <el-row :gutter="10" align="middle">
              ...
              <el-col :span="4">
                ...
                <div class="form-hint">{{ $t('rules.probabilityHint') }}</div>
              </el-col>
              ...
              <el-col :span="4" class="fault-actions">
                <el-button size="small" type="danger" text @click="removeFault(fIdx)" v-if="authStore.canWriteRule">{{ $t('rules.deleteFault') }}</el-button>
              </el-col>
            </el-row>
          </div>
          <el-button size="small" @click="addFault" v-if="authStore.canWriteRule" class="add-fault-btn">{{ $t('rules.addFault') }}</el-button>
        </div>
      </el-card>
      ...
      <div class="form-actions">
        ...
        <span v-if="!authStore.canWriteRule" class="no-permission">{{ $t('rules.noPermission') }}</span>
      </div>
    </el-form>
  </div>
</template>
...
.inline-switch :deep(.el-form-item__label) {
  font-weight: 600;
...
.response-name-input :deep(.el-input__inner) {
  font-weight: 600;
}
.response-card-tags {
  display: flex;
...
.response-subsection:last-child {
  margin-bottom: 0;
}
.subsection-header {
  display: flex;
